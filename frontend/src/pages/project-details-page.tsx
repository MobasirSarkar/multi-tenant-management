...
export const ProjectDetailsPage = () => {
    ...
    return (
        <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd} sensors={sensors}>
            <div className="min-h-screen bg-gray-50/50 p-6 font-mono">
                {/* Header */}
                <header className="mb-8 border-b-2 border-black pb-4">
                    <Link to="/" className="inline-flex items-center text-sm font-bold uppercase hover:underline mb-4">
                        <ArrowLeft className="mr-2 h-4 w-4" /> Back to Dashboard
                    </Link>

                    <div className="flex justify-between items-start">
                        <div>
                            <h1 className="text-4xl font-black uppercase tracking-tighter mb-2">{project.name}</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[calc(100vh-250px)]">
                    {COLUMNS.map((col) => {
                        // Filter tasks for this column based on LOCAL state
                ...
                <DragOverlay>
                    {activeId && activeTask ? (
                        <div className="rotate-3 opacity-90 cursor-grabbing"> {/* Added generic tilt for effect */}
